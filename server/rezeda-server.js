/* =============================================================================
   ...
   Запуск: двойной клик по «Запустить сервер данных.cmd» в этой самой папке
   хранения (или node server/rezeda-server.js).
   ...
   Где лежат данные: соседняя с проектом папка «КП автоматизация Резеда -
   сервер» (dogovor-data.json) — НАМЕРЕННО вне папки проекта, чтобы
   юридические данные (ИНН, паспорта, банковские реквизиты) никогда не
   попали в git и на GitHub.
   ============================================================================= */
"use strict";
...
// Папка хранения — рядом с проектом, а не внутри него: юридические данные
// не должны попасть в git. Переопределяется переменной REZEDA_DATA_DIR.
const DATA_DIR = process.env.REZEDA_DATA_DIR ||
  path.resolve(ROOT, "..", "КП автоматизация Резеда - сервер");
