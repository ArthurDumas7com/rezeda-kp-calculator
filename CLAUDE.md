...
- **External access** is an SSH tunnel through localhost.run (`ssh -R 80:localhost:8765 nokey@localhost.run`), not Cloudflare: the bureau's corporate VPN blocks outbound 7844, so `cloudflared` fails at the TLS handshake. Also note that VPN's DNS returns synthetic 198.19.x.x addresses, which breaks SRV discovery.
